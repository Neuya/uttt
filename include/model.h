...
/*!
 * This function saves the state of the corresponding p_uttt into a file referred by p_f.
 * The first line of the file contains the inception level of the uttt.
 * The following line represent the move history of the uttt as inner position, outer position and the player.
 * Positions are labeled according to the following set {"TL", "TC", "TR", "ML", "MC", "MR", "BL", "BC", "BR", "FR", "NO"}
 * Player are labeled according to the following set {"X", "O", "#", "."}
 *
 * \param p_f an a file descriptor to write in
 * \param p_uttt a pointer on a s_utictactoe.
 */
void save_a_utictactoe_to_file(FILE *p_f, s_utictactoe *p_uttt);

e_status undo_last_move(s_utictactoe*);
